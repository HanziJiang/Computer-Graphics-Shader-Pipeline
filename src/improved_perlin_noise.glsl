...
float improved_perlin_noise( vec3 st) 
{
  ...
  vec3 st_lower = floor(st);
  vec3 st_upper = st_lower + 1;
  vec3 st_diff = st - st_lower;
  ...
  vec3 smooth_st_diff = vec3(improved_smooth_step(st_diff.x), improved_smooth_step(st_diff.y), improved_smooth_step(st_diff.z));
